...
export class Store {
  constructor(options = {}) {
    ...
    /**
     * 根据传入的参数注册的基本模块;
     * 此时只是注册、建立好了各个模块的关系，已经定义了各个模块的state状态，但getters、mutations等方法暂未注册
     */
    this._modules = new ModuleCollection(options) // 根据传入的options配置，注册各个模块，
    ...
    // 生成一个Vue实例去管理state状态，同时将getters交给computed处理，通过VM使store“响应式”
  ...
  /**
   * commit 方法
   * @param {*} _type mutation类型名称 
   * @param {*} _payload 预载(参数)
   * @param {*} _options 选项(比如root:true)
   */
  commit(_type, _payload, _options) {
    ...
    const entry = this._mutations[type] // 查找this._mutations上是否有对应的方法
    // 查找不到则不执行任何操作
    if (!entry) {
      //在开发时打印错误
      if (__DEV__) {
    ...
    this._subscribers
      .slice() //slice()返回新的数组，该数组是原数组的浅拷贝。浅表复制，以防止如果订户同步调用取消订阅，则迭代器无效
  ...
  /**
   * // 注册新模块，store.registerModule('myModule', {...})
   * @param {*} path 
   * @param {*} rawModule 
   * @param {*} options
   * 
  ...
  /**
   * 卸载模块 // 注册嵌套模块 `nested/myModule`store.registerModule(['nested', 'myModule'], {...})
   * @param {*} path 
   */
...
/**
 * installModule的作用主要是用为module加上namespace名字空间（如果有）后，
...
/**
 * 处理两种代码提交风格的情况，
 * 例如：1、this.$store.dispatch({type: 'add', count: 1}); 2、this.$store.dispatch('add', {count: 1})
 * @param {*} type 
 * @param {*} payload 
 * @param {*} options 
 */
